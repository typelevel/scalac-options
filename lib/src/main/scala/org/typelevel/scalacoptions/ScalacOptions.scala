...
private[scalacoptions] trait ScalacOptions {
  ...
  val fatalWarnings =
    advancedOption(
      "fatal-warnings",
      version => version < V2_13_0 || version.isBetween(V3_0_0, V3_0_2)
    )
  ...
  val warnError =
    warnOption(
      "error",
      version => version.isBetween(V2_13_0, V3_0_0) || version.isAtLeast(V3_0_2)
    )

  /** Unused warning options (-Wunused:)
    */
  def warnUnusedOption(
    name: String,
    isSupported: ScalaVersion => Boolean = _ => true
  ) =
  ...
  /** Enable cross-method optimizations.
    ...
    */
  def optimizerOptions(inlineFromPackages: String*): Set[ScalacOption] =
    ListSet(
  ...
  def optionsForVersion(
    ...
    proposedScalacOptions.filter(_.isSupported(scalaVersion))

  def tokensForVersion(
    scalaVersion: ScalaVersion,
    proposedScalacOptions: Set[ScalacOption]
  ): Seq[String] =
    optionsForVersion(scalaVersion, proposedScalacOptions).toList.flatMap(opt =>
      opt.option :: opt.args
    )

  def defaultOptionsForVersion(scalaVersion: ScalaVersion): Set[ScalacOption] =
    optionsForVersion(scalaVersion, default)

  def defaultTokensForVersion(scalaVersion: ScalaVersion): Seq[String] =
    tokensForVersion(scalaVersion, default)
}
